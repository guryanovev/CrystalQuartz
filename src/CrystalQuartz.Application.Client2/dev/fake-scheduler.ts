...
export class FakeScheduler {
  ...
  init() {
    const mapJob = (name: string, data: ScheduleJob) =>
      new Job(
        name,
        data.duration,
        Object.keys(data.triggers).map((key) =>
          this.mapTrigger(key, data.duration, data.triggers[key])
        )
      );
    const mapJobGroup = (name: string, data: ScheduleGroup) =>
      new JobGroup(
        name,
        Object.keys(data).map((key) => mapJob(key, data[key]))
      );

    this._groups = Object.keys(this.schedule).map((key) => mapJobGroup(key, this.schedule[key]));

    this._triggers = this._groups.flatMap((g) => g.jobs.flatMap((j) => j.triggers));
  }
  ...
  private doStateCheck() {
    ...
    const now = new Date().getTime();
    const triggersToStop = this.inProgress.filter((item) => {
    ...
    if (this.status === SchedulerStatus.Started) {
      const triggersToStart = this._triggers.filter((trigger) => {
        return (
          trigger.status === ActivityStatus.Active &&
          !trigger.isDone() &&
    ...
    if (this.inProgress.length > 0) {
      nextUpdateAt = Math.min(
        ...this.inProgress.map((item) => item.startedAt + item.trigger.duration)
      );
    }

    const activeTriggers = this._triggers.filter(
      (trigger) => trigger.status === ActivityStatus.Active && trigger.nextFireDate
    );
    if (this.status !== SchedulerStatus.Shutdown && activeTriggers.length > 0) {
      ...
      nextUpdateAt =
        nextUpdateAt === null ? nextTriggerFireAt : Math.min(nextUpdateAt, nextTriggerFireAt);
    }
    ...
    if (nextUpdateAt === null) {
      ...
      this._timer.schedule(() => this.doStateCheck(), nextUpdateIn);
    }
  }
  ...
  private pushEvent(
    ...
    this._events.push({
      ...
      faulted: faulted,
      errors: faulted
        ? [new ErrorMessage(0, 'Test exception text'), new ErrorMessage(1, 'Inner exception text')]
        : null,
    });
  ...
  private changeJobStatus(groupName: string, jobName: string, status: ActivityStatus) {
    ...
    if (group) {
      ...
      if (job) {
        ...
        this.doStateCheck();
        this.pushEvent(
          SchedulerEventScope.Job,
          this.getEventTypeBy(status),
          group.name + '.' + job.name
        );
      }
    }
  ...
  shutdown() {
    ...
    alert(
      'Fake in-browser scheduler has just been shut down. Just refresh the page to make it start again!'
    );
  }
