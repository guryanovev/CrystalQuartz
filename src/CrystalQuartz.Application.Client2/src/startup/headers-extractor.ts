...
export const ANALYZE_LOCATION = (): LocationAnalyzingResult => {
  try {
    if (window && window.parent) {
      const options: CrystalQuartzParentOptions | undefined = (
        window.parent as unknown as CrystalQuartzParentOptionsHolder
      )['CQ_OPTIONS'];
    ...
    if (location.search) {
      ...
      if (found) {
        return {
          headers: result,
          url:
            location.pathname + (restParameters.length === 0 ? '' : '?') + restParameters.join('&'),
        };
      }
    }
  } catch (ex) {}
